document.addEventListener("DOMContentLoaded", function () {
    fetchDataTNFromServer();
    checkuserlogin();
});

function fetchDataTNFromServer() {
    fetch('http://localhost:5000/cateTN')
        ...
        .then(data => {
            if (Object.keys(data).length === 0) {
                ...
                console.log(data);
                sessionStorage.setItem('categoryTNData', JSON.stringify(data));
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    sortLinks.forEach(function (slink) {
        slink.addEventListener('click', function (event) {
            ...
            if (slinkID === "ASC") {
                var SortData = JSON.parse(sessionStorage.getItem('categoryTNData'));
                if (SortData) {
                    ...
                    sessionStorage.setItem('categoryTNData', JSON.stringify(SortData));
                    SortData = JSON.parse(sessionStorage.getItem('categoryTNData'));
...
function displayProducts(page) {
    var categoryTNData = JSON.parse(sessionStorage.getItem('categoryTNData'));
    if (categoryTNData) {
        const startIndex = (page - 1) * itemsPerPage;
        const endIndex = startIndex + itemsPerPage;
        const paginatedProducts = categoryTNData.slice(startIndex, endIndex);
        var container = document.getElementById('dssp');
        container.innerHTML = ''
...
function setupPagination() {
    var categoryTNData = JSON.parse(sessionStorage.getItem('categoryTNData'));
    console.log(categoryTNData)
    const totalPages = Math.ceil(categoryTNData.length / itemsPerPage);
