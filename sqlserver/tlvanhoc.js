document.addEventListener("DOMContentLoaded", function () {
    fetchDataVHFromServer();
    // checkuserlogin();
});

function fetchDataVHFromServer() {
    fetch('http://localhost:5000/cateVH')
        ...
        .then(data => {
            if (Object.keys(data).length === 0) {
                ...
                console.log(data);
                sessionStorage.setItem('categoryVHData', JSON.stringify(data));
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    sortLinks.forEach(function (slink) {
        slink.addEventListener('click', function (event) {
            ...
            if (slinkID === "ASC") {
                var SortData = JSON.parse(sessionStorage.getItem('categoryVHData'));
                if (SortData) {
                    ...
                    sessionStorage.setItem('categoryVHData', JSON.stringify(SortData));
                    SortData = JSON.parse(sessionStorage.getItem('categoryVHData'));
...
function displayProducts(page) {
    var categoryVHData = JSON.parse(sessionStorage.getItem('categoryVHData'));
    if (categoryVHData) {
        const startIndex = (page - 1) * itemsPerPage;
        const endIndex = startIndex + itemsPerPage;
        const paginatedProducts = categoryVHData.slice(startIndex, endIndex);
        var container = document.getElementById('dssp');
        container.innerHTML = ''
...
function setupPagination() {
    var categoryVHData = JSON.parse(sessionStorage.getItem('categoryVHData'));
    console.log(categoryVHData)
    const totalPages = Math.ceil(categoryVHData.length / itemsPerPage);
