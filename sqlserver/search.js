...
function fetchDataSearchFromServer(BookName) {
    ...
    fetch('http://localhost:5000/searchdsbook', {
        method: 'POST',
        headers: {
        ...
        .then(data => {
            if (Object.keys(data).length === 0) {
                ...
                console.log(data);
                sessionStorage.setItem('searchdsall', JSON.stringify(data));
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    sortLinks.forEach(function (slink) {
        slink.addEventListener('click', function (event) {
            ...
            if (slinkID === "ASC") {
                var SortData = JSON.parse(sessionStorage.getItem('searchdsall'));
                if (SortData) {
                    ...
                    sessionStorage.setItem('searchdsall', JSON.stringify(SortData));
                    SortData = JSON.parse(sessionStorage.getItem('searchdsall'));
...
function displayProducts(page) {
    var searchdsall = JSON.parse(sessionStorage.getItem('searchdsall'));
    if (searchdsall) {
        const startIndex = (page - 1) * itemsPerPage;
        const endIndex = startIndex + itemsPerPage;
        const paginatedProducts = searchdsall.slice(startIndex, endIndex);
        var container = document.getElementById('dssp');
        container.innerHTML = ''
...
function setupPagination() {
    var searchdsall = JSON.parse(sessionStorage.getItem('searchdsall'));
    console.log(searchdsall)
    const totalPages = Math.ceil(searchdsall.length / itemsPerPage);
