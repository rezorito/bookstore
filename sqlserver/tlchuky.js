...
function fetchDataCKFromServer() {
    fetch('http://localhost:5000/cateCK')
        ...
        .then(data => {
            if (Object.keys(data).length === 0) {
                ...
                console.log(data);
                sessionStorage.setItem('categoryCKData', JSON.stringify(data));
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    sortLinks.forEach(function (slink) {
        slink.addEventListener('click', function (event) {
            ...
            if (slinkID === "ASC") {
                var SortData = JSON.parse(sessionStorage.getItem('categoryCKData'));
                if (SortData) {
                    ...
                    sessionStorage.setItem('categoryCKData', JSON.stringify(SortData));
                    SortData = JSON.parse(sessionStorage.getItem('categoryCKData'));
...
function displayProducts(page) {
    var categoryCKData = JSON.parse(sessionStorage.getItem('categoryCKData'));
    if (categoryCKData) {
        const startIndex = (page - 1) * itemsPerPage;
        const endIndex = startIndex + itemsPerPage;
        const paginatedProducts = categoryCKData.slice(startIndex, endIndex);
        var container = document.getElementById('dssp');
        container.innerHTML = ''
...
function setupPagination() {
    var categoryCKData = JSON.parse(sessionStorage.getItem('categoryCKData'));
    console.log(categoryCKData)
    const totalPages = Math.ceil(categoryCKData.length / itemsPerPage);
