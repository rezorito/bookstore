...
function fetchDataVPPFromServer() {
    fetch('http://localhost:5000/cateVPP')
        ...
        .then(data => {
            if (Object.keys(data).length === 0) {
                ...
                console.log(data);
                sessionStorage.setItem('categoryVPPData', JSON.stringify(data));
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    sortLinks.forEach(function (slink) {
        slink.addEventListener('click', function (event) {
            ...
            if (slinkID === "ASC") {
                var SortData = JSON.parse(sessionStorage.getItem('categoryVPPData'));
                if (SortData) {
...
function displayProducts(page) {
    var categoryVPPData = JSON.parse(sessionStorage.getItem('categoryVPPData'));
    if (categoryVPPData) {
        const startIndex = (page - 1) * itemsPerPage;
        const endIndex = startIndex + itemsPerPage;
        const paginatedProducts = categoryVPPData.slice(startIndex, endIndex);
        var container = document.getElementById('dssp');
        container.innerHTML = ''
...
function setupPagination() {
    var categoryVPPData = JSON.parse(sessionStorage.getItem('categoryVPPData'));
    console.log(categoryVPPData)
    const totalPages = Math.ceil(categoryVPPData.length / itemsPerPage);
