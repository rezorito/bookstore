...
app.get('/ctsp', (req, res) => {
    ...
    request.query(`SELECT ID as BookID, 
               Name as BookName, 
               Price as BookPrice, 
               Sale as BookSale, 
               ...
               Image as BookImage, 
               Count as BookCount
                FROM Book WHERE ID = @bookID`, (err, recordset) => {
        if (err) {
            console.error('Lỗi truy vấn:', err);
            res.status(500).send('Lỗi truy vấn cơ sở dữ liệu.');
            return;
...
app.post('/add-product', (req, res) => {
    ...
    request.query('INSERT INTO Book (ID, Name, Price, Category_ID, Sale, Content, Image) VALUES (@BookID, @BookName, @BookPrice, @TheLoaiID, @BookSale, @BookContent, @BookImage);', (err, result) => {
        if (err) {
            console.error('Lỗi truy vấn:', err);
...
app.put('/fix-product', (req, res) => {
    ...
    request.input('BookImage', sql.VarChar, BookImage);
    request.input('TheLoaiID', sql.Char, TheLoaiID);
    request.query('UPDATE Book SET Name = @BookName, Price = @BookPrice, Sale = @BookSale, Content = @BookContent, Image = @BookImage, Category_ID = @TheLoaiID WHERE ID = @BookID; ', (err, recordset) => {
        if (err) {
            console.error('Lỗi truy vấn:', err);
            res.status(500).send('Lỗi truy vấn cơ sở dữ liệu.');
            return;
